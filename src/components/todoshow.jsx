import React from 'react'
import { useNavigate } from 'react-router-dom';
import { db  } from '../config/firebase/firebaseconnection';
import { useState, useEffect } from 'react';
import { collection , deleteDoc, doc, getDocs } from 'firebase/firestore';


function Todoshow() {

    const [commentShow, setCommentShow] = useState([]);
    const [commentDelete, setCommentDelete] = useState(null);
    const [isLoader, setIsLoader] = useState(false);

    const navigate = useNavigate();

    const fetchData = async () => {
        setIsLoader(true);
        try {
            const commentSnapshot = await getDocs(collection(db, 'Todo Data'));
            const comments = commentSnapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data()
            }));
            setCommentShow(comments);
            setIsLoader(false);
        } catch (err) {
            setIsLoader(false);
            console.log(err);
        }
    };

    useEffect(() => {
        fetchData();
    }, [commentDelete]);

    const handleAddComment = () => {
        navigate("/todoform");
    };

    const handleDelete = async (id) => {
        try {
            await deleteDoc(doc(db, "Todo Data", id));
            setCommentDelete(id);
            console.log("Comment Deleted: ", id);
        } catch (err) {
            console.log("Error: ", err);
        }
    };


  return (

    <>
    <div className='bg-green-800 py-6 px-12 flex justify-between rounded-full items-center'>
        <h2 className="text-4xl font-bold  text-white">FAQ'S</h2>
        <button
            className='px-6 py-2 text-white bg-black font-bold rounded-lg hover:bg-gray-300 shadow-md'
            onClick={handleAddComment}>
            Add your Question
        </button>
    </div>

    <div className="mx-12 my-8 px-80">
        {isLoader ? (
            <div className='flex justify-center  items-center w-full'>
                <img className='w-100' src="data:image/png;base64," alt="Loading..." />
            </div>
        ) : (
            <div className="flex flex-col p-4 border-4 border-green-900 gap-6 mt-8 items-center">
                {commentShow.map(comment => (
                    <div
                        key={comment.id}
                        className="w-full  md:w-2/3 lg:w-4/5 p-6 bg-white border-2 border-green-800 rounded-lg shadow-lg relative">
                        
                        <h3 className="text-xl font-bold text-black">Subject : {comment.name}</h3>
                        <p className="text-black text-lg mt-2">Question : {comment.comment}</p>

                        <div className=' relative top-5 flex gap-3'>
                            <button
                                className='p-2 font-semibold bg-red-800 text-white rounded-lg hover:bg-red-600'
                                onClick={() => handleDelete(comment.id)}>
                               Delete
                            </button>
                            <button
                                className='p-2 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600'
                                onClick={() => navigate("/ProductsForm")}>
                                Edit
                            </button>
                        </div>
                    </div>
                ))}
            </div>
        )}
    </div>
</>

  )
}

export default Todoshow
